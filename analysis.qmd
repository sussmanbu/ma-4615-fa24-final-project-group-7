...
binary_data <- police_interaction %>%
  select(all_of(binary_columns)) %>%
  select(where(is.numeric))

categorical_data <- police_interaction %>%
  select(all_of(categorical_columns)) %>%
  select(where(is.numeric))

num_categorical_data <- police_interaction %>%
...
```
One notable positive correlation from the graph above is the relation between NUM_IMPROPER, the number of people in households who experienced improper contact, and NUM_PRO_PERS_HH, the number of people in households with suspect-related police-initiated contact.
