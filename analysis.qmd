...
Our data analysis is performed on the combined data from two datasets. One of these datasets is titled "The Effect of Prior Police Contact on Victimization Reporting...", and it reports national data about civilian experience with the police from the DOJ's National Crime Victimization Survey and the Police-Public Contact Survey. The other dataset, titled "Firearm Suicide Proxy for Household Gun Ownership" examines gun ownership rates and homocide rates for both white and non-whites in different states of the US. We want to use these datasets to better understand the racial disparities underlying police interactions and how the interactions might be shaped by gun statistics.

The variables we are most interested in analyzing are the rate of improper police interactions and the rate at which people are arrested, and we want to determine how those are impacted by factors such as race (in particular), education, household income, sex, and the homocide rate & gun ownership levels in different regions of the US.
Some questions:
How does the rate of improper police interactions vary among populations of different races?
How does the homocide rate vary by region and by time period (from 2002 to 2011)?
...
This is useful to understand since we can use this information to dig deeper into the causes behind negative police behavior. Specifically, this may help us determine whether race or the level of neighborhood gun violence is a better indicator for particular aspects of police behavior in our study.
