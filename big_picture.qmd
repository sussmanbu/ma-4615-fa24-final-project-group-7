...
# region_shp <- st_read("https://sussmanbu.github.io/ma-4615-fa24-final-project-group-7/dataset_for_shiny/region_shapefile.shp")
...
region_shp <- st_read("dataset/region_shapefile.shp")
region_stats <- read_rds(here::here("dataset/combined_regional_data.rds"))

region_shp <- region_shp |>
  filter(!is.na(region))

region_stats$region <- str_to_title(region_stats$region)

#2002
stats_2002 <- region_stats |>
  filter(year == "2002")

shp_data_2002 <- region_shp |>
  left_join(stats_2002, by = c("region" = "region"))

#2008
stats_2008 <- region_stats |>
  filter(year == "2008")

shp_data_2008 <- region_shp |>
  left_join(stats_2008, by = c("region" = "region"))

#2011
stats_2011 <- region_stats |>
  filter(year == "2011")

shp_data_2011 <- region_shp |>
  left_join(stats_2011, by = c("region" = "region"))

ggplot(data = shp_data_2011) +
  geom_sf(aes(fill = population)) +  # Color based on population
  scale_fill_gradient(low = "lightblue", high = "darkblue") +  # Gradient scale for population
  labs(title = "Population by Region in the US", fill = "Population") +
  theme_minimal()
```

This population map of the United States allows readers to explore demographic data visually. Each region is shaded according to its population, with darker tones representing higher population concentrations. The southeastern and southern regions of the U.S. have the highest population densities, as they are shaded in the darkest blue. In contrast, the western and northern regions show lighter shades, signifying lower population concentrations.The color scale on the right provides a clear reference for interpreting the population values, ranging from approximately 7.0e+07 to 1.0e+08, helping the viewer understand the relative differences between regions at a glance. 
...
server <- function(input, output) {
  ...
  output$map <- renderLeaflet({
    ...
    leaflet(shp_data_2002) %>%
      addProviderTiles(providers$CartoDB.Positron) %>%
      addPolygons(
        fillColor = ~pal(region), # Use stat1 for coloring
        fillOpacity = 0.7,
        color = "black",
        weight = 1,
        popup = ~paste("Region: ", region, "<br>",
                      ...
                      "FA Homicide Rate: ", fa_homicide_rate, "<br>"),
